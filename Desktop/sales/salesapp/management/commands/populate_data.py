...
def partition_installments(remaining, total_months):
    ...
    if num_systems == 1:
        ...
        if rem_2 <= 0: return partition_installments(remaining, total_months) # fallback
        ...
        last_val = remaining - sum(amounts)
        if last_val <= 0: return partition_installments(remaining, total_months) # fallback
        amounts.append(last_val)
...
class Command(BaseCommand):
    help = 'Populates the database with realistic data (Jan 2025 to May 2026), V2 schema, divisible by 5.'
    ...
    def get_next_receipt_number(self):
        if self.last_receipt_number == 0:
            ...
            self.last_receipt_number = int(max_num) if max_num else 0
        self.last_receipt_number += 1
        return self.last_receipt_number

    def handle(self, *args, **kwargs):
        self.stdout.write(self.style.WARNING("بدء مسح وبناء قاعدة البيانات..."))
        ...
        # 1. Branch
        branch = Branch.objects.create(name="السنبلاوين")
        
        # 2. Salespersons
        salesperson_names = ["أحمد السيد", "محمود علي", "يوسف إبراهيم", "طارق حسن"]
        salespersons = [Salesperson.objects.create(name=name, branch=branch) for name in salesperson_names]
        
        # 3. Suppliers
        suppliers = [
            Supplier.objects.create(name="مصنع المجد للأدوات"),
            Supplier.objects.create(name="مؤسسة التوحيد للمفروشات")
        ]

        # 4. Products & Initial V2 Setup
        inventory_items = []
        product_info_lookup = {}
        for prod_name, sale_price in PRODUCTS:
            purchase_price = round_to_5(sale_price * 0.5)
            # Commission logic: proportional to price (30 to 120), round to 5
            commission = round_to_5(min(max(30, sale_price // 30), 120))
            
            # Initial quantity 10 to 50
        ...
        self.stdout.write(f"تم إنشاء {len(inventory_items)} منتج بنظام V2 والأرصدة الافتتاحية المنطقية.")

        # 5. Generate Sales (1/2025 to 5/2026)
        ...
        while current_date <= end_date:
            ...
            # --- Replenish Stock via Purchase Invoices ---
            # Every month, we check inventory and buy more to ensure we don't run out.
            # We'll just randomly buy some items every month to keep quantities healthy.
            items_to_buy = random.sample(inventory_items, 20)
            for supplier in suppliers:
                # generate an integer invoice number
                inv_num_str = f"{target_year}{target_month:02d}{supplier.id}{random.randint(100,999)}"
            ...
            while receipts_this_month < target_receipts:
                with transaction.atomic():
                    is_cash = random.random() < 0.01 # 1% cash
                    ...
                    for item in selected_items:
                        # Skip full inventory checking since we're generating dummy data and replenishing monthly.
                        # The system computes current quantity dynamically now.
                    ...
                    if not is_cash:
                        ...
                        if remaining > 0:
                            # 12 to 15 months based on invoice size
                            months = 12
