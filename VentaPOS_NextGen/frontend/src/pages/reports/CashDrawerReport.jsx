      ...
      <div className="page-header d-print-none">
        <div className="container-fluid">
          <div className="row g-2 align-items-center">
            <div className="col">
              <h2 className="page-title">درج الخزينة</h2>
                ...
                <table className="table table-hover table-striped text-center align-middle mb-0">
                  ...
                  <tbody>
                    {combinedInvoices.length > 0 ? combinedInvoices.map((item, idx) => (
                      <tr key={idx}>
                        <td className="fw-bold text-primary">#{item.receipt_number}</td>
                        <td className="fw-bold">{item.customer_name}</td>
                        <td className="fw-bold">{item.type === 'cash' ? fmt(item.amount) : fmt(item.total_amount)}</td>
                        <td className="text-success fw-bold">{item.type === 'cash' ? 'كاش' : fmt(item.down_payment)}</td>
                    ...
                    {data.collections && data.collections.length > 0 ? data.collections.map((item, idx) => {
                      ...
                      return (
                        <tr key={idx}>
                          <td className="fw-bold text-primary">#{item.receipt_number}</td>
                          <td className="fw-bold">{item.customer_name}</td>
                          <td className="text-success fw-bold">{fmt(item.amount)}</td>
                  ...
                  <tfoot className="table-success fw-bold">
                    <tr>
                      ...
                      <td colSpan="2" className="text-start fs-5">
                        {fmt((data.collections || []).reduce((acc, item) => acc + parseInt(item.amount || 0), 0))}
                      </td>
                    </tr>
                  </tfoot>
                </table>
